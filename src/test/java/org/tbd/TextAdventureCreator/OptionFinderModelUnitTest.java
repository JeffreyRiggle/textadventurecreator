...
public class OptionFinderModelUnitTest {
	...
	private OptionPersistenceObject option1;
	private OptionPersistenceObject option2;
	private OptionPersistenceObject option3;
	
	private OptionFinderModel model;
	
	@Before
	public void setup() {
		libraryService = mock(LibraryService.class);
		item1 = mock(LibraryItem.class);
		option1 = mock(OptionPersistenceObject.class);
		when(item1.options()).thenReturn(Arrays.asList(option1));
		when(item1.getLibraryName()).thenReturn("Lib1");
		when(item1.getAuthor()).thenReturn("Tester");
		
		item2 = mock(LibraryItem.class);
		option2 = mock(OptionPersistenceObject.class);
		option3 = mock(OptionPersistenceObject.class);
		when(item2.options()).thenReturn(Arrays.asList(option2, option3));
		...
		model = new OptionFinderModel(libraryService, languageService, dialogService, actionViewFactory, triggerViewFactory, dialogProvider, styleService, urlProvider);
	}
	...
	public void testSearchAnyField() {
		...
		assertTrue(model.results().list().contains(option1));
		assertTrue(model.results().list().contains(option2));
		assertTrue(model.results().list().contains(option3));
		
		model.searchText().set("P");
		assertFalse(model.results().list().contains(option1));
		assertTrue(model.results().list().contains(option2));
		assertTrue(model.results().list().contains(option3));
	}
	...
	public void testSearchLibraryName() {
		...
		assertTrue(model.results().list().contains(option1));
		assertFalse(model.results().list().contains(option2));
		assertFalse(model.results().list().contains(option3));
